...
设$z$和$w$是两个复数，那么
\begin{eenum}
  \item $\Re z=\frac12(z+\bar z),\Im z=\frac1{2i}(z-\bar z)$;\label{prop1.1.3.1}
...
在平面上取定一个直角坐标系，实数对$(a,b)$就表示平面上的一个点，所以复数$z=a+bi$可以看成平面上以$a$为横坐标、以$b$为纵坐标的一个点（图 \ref{fig1.1}）.这个点的极坐标设为$(r,\theta)$，那么
\begin{figure}[!ht]
\centering
...
$ \sqrt[\leftroot{-1}\uproot{2} n]w,\sqrt[\leftroot{-1}\uproot{2} n]w\omega,\cdots,\sqrt[\leftroot{-1}\uproot{2} n]{w}\omega^{n-1}.$ 
 ...
 \textbf{Exercise: }\hypertarget{xiti1.2}{}
 \item 把复数$z=1+\cos\theta+i\sin\theta$写成三角形形式.
 \item 问$n$取何值时有$(1+i)^n=(1-i)^n$ ?
 \item 证明：
