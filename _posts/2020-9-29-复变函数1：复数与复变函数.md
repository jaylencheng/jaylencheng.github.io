...
**Definition:** \label{def1.1.1}
域$\mathbb{F}$称为\textbf{有序域}，如果在$\mathbb{F}$的元素间能确定一种关系（记为$a<b$），其满足下列要求：

  * \label{def1.1.1.1}对$\mathbb{F}$中任意两个元素$a,b$, 下述三个关系中必有而且只有一个成立：
  $$ a<b,\quad a=b,\quad b<a; $$
    
  * \label{def1.1.1.2}如果$a<b,b<c$，那么$a<c$；

  * \label{def1.1.1.3}如果$a<b$，那么对任意$c$，有$a+c<b+c$；

  * \label{def1.1.1.4}如果$a<b,c>0$，那么$ac<bc$.

容易知道，实数域是有序域，而复数域则不是.
**Throrem: ** 复数域不是有序域.

**Proof:**
如果$\mathbb{C}$是有序域，那么因为$i\ne0$， $i$与$0$之间必有$i>0$或$i<0$的关系.如果$i>0$， 则由 \ref{def1.1.1.4} 得$i\cdoti>i\cdot0$， 即$-1>0$,再由 \ref{def1.1.1.3}， 两端都加$1$,即得$0>1$.另一方面，从$-1>0$还可得$-1\cdot(-1)>0\cdot(-1)$,即$1>0$，这和刚才得到的$0>1$矛盾.如果$i<0$，两端都加$-i$，再由 \ref{def1.1.1.4}，两端乘$-i$，得$-1>0$.重复上面的讨论，即可得$0>1$和$0<1$的矛盾.所以，复数域不是有序域.

从现在开始，我们不再用实数对$(a,b)$来记复数，而直接用$z=a+bi$记复数，$a$称为$z$的\textbf{实部}\index{F!复数!实部}，$b$称为$z$的\textbf{虚部}\index{F!复数!虚部}，分别记为$a=\Re z,b=\Im z$.加法和乘法用现在的记号定义为:
...
$$
&(a+bi)-(c+di)=(a-c)+(b-d)i,\\
&\frac{a+bi}{c+di}=(a+bi)\bigg(\frac{c-di}{c^2+d^2}\bigg)
...
$|z|$称为$z$的**模**或绝对值，$\bar z$称为$z$的**共轭复数**.下面是它们的一些基本性质：

**Property:**
设$z$和$w$是两个复数，那么

  *  $\Re z=\frac12(z+\bar z),\Im z=\frac1{2i}(z-\bar z)$;
  *  $z\bar z=|z|^2$;
  *  $\overline{z+w}=\bar z+\bar w,\overline{zw}=\bar z\bar w$;
  *  $|zw|=|z|\,|w|,\left|\frac zw\right|=\frac{|z|}{|w|}$;
  *  $|z|=|\bar z|$.

这些性质的证明都很简单，但在证明4时，初学者往往会用$z$和$w$的实部和虚部来表示$|zw|$和$|z|\,|w|$，从而证明它们相等. 其实，利用 2来证明要简单得多：
$$|zw|^2=(zw)(\overline{zw})=|z|^2|w|^2.$$

**Property: **\label{prop1.1.4}
设$z$和$w$是两个复数，那么

  *  $|\Re z|\le |z|,|\Im z|\le |z|$;\label{prop1.1.4.1}
  *  $|z+w|\le|z|+|w|$，等号成立当且仅当存在某个$t\ge0$,使得$z=tw$;\label{prop1.1.4.2}
  *  $|z-w|\ge\bigl||z|-|w|\bigr|$.\label{prop1.1.4.3}
...
1. 设$z_1,\cdots,z_n$是任意$n$个复数，证明：
...
3.  若$|z_1|=\lambda|z_2|,\lambda>0$,证明：
...
5.  设$|a|<1,|z|<1$，证明：
     ...
     *  $1-\bigg|\frac{z-a}{1-\bar az}\bigg|^2=\frac{\bigl(1-|a|^2\bigr)\bigl(1-|z|^2\bigr)}{|1-\bar az|^2}$；

     *  $\frac{\bigl||z|-|a|\bigr|}{1-|a|\,|z|}\le\bigg|\frac{z-a}{1-\bar az}\bigg|\le
       \frac{|z|+|a|}{1+|a|\,|z|}$.

6.  设$z_1,\cdots,z_n,w_1,\cdots,w_n$是任意$2n$个复数，证明复数形式的\textbf{Lagrange等式}：\index{L!Lagrange等式}，并由此推出\textbf{Cauchy不等式}。不等式中等号成立的条件是什么?

7.  设$z_1,\cdots,z_n$是任意$n$个复数，证明必有$\{1,2,\cdots,n\}$的子集$E$，使得
...
我们还可把复数$z=a+bi$看成在$x$轴和$y$轴上的投影分别为$a$和$b$的一个向量，这时我们就把复数和向量作为同义语来使用.容易知道，由一向量经过平行移动所得的所有向量表示的是同一个复数.如果一个向量的起点和终点分别为复数$z_1$和$z_2$，那么这个向量所表示的复数便是$z_2-z_1$，因而$|z_2-z_1|$就表示$z_1$与$z_2$之间的距离.特别地，当一个向量的起点为原点时，它的终点所表示的复数和向量所表示的复数是一致的.

前面定义的复数的加法和向量的加法是一致的：把两个不重合的非零向量$z_1$和$z_2$的起点取在原点，以$z_1$和$z_2$为两边作平行四边形，那么以原点为起点沿对角线所作的向量就表示$z_1+z_2$；以$z_2$为起点，$z_1$为终点的向量就表示$z_1-z_2$（图 \ref{fig1.2}）.现在再来看 1.1 节命题 \ref{prop1.1.4} 中 \ref{prop1.1.4.2} 的不等式$|z_1+z_2|\le|z_1|+|z_2|$，它实际上就是三角形两边之和大于第三边的最简单的几何命题.
...
第一个等式在 \ref{sec1.1} 节中已经证明过；第二个等式应该理解为两个集合的相等.这就是说，两个复数的乘积是这样一个复数，它的模是两个复数的模的乘积，它的辐角是两个复数的辐角之和.从几何上看，用复数$w$乘复数$z$，相当于把$z$沿反时针方向转动大小为$\arg w$的角，再让$z$的长度伸长$|w|$倍.特别地，如果$w$是单位向量，那么$w$乘$z$的结果就是把$z$沿反时针方向转动大小为$\arg w$的角.例如，已知$i$是单位向量，它的辐角为$\frac\pi2$，因此$i z$就是把$z$按反时针方向转动$\frac\pi2$角所得的向量.这种几何直观在考虑问题时非常有用.
...
**Proof:**
把$A$取作坐标原点，$AB$所在的直线取作$x$轴，那么$P,Q$的坐标分别为$a$和$a+h$.如果用$\ee^{i\theta} $记$\cos\theta+i\sin\theta$，那么$R$点和$S$点可分别用复数$r\ee^{i\theta}$和$(r+h)\ee^{i\theta}$表示.由于$M$和$N$分别是$PR$和$SQ$的中点，所以$M$和$N$可以分别用复数表示为

$$
  &M:\frac12\big(a+r\ee^{i\theta}\big),\\
  &N:\frac12\big[(a+h)+(r+h)\ee^{i\theta}\big].
$$

若记$z_1=\overrightarrow{MN}$，则

$$z_1=\frac12\big[(a+h)+(r+h)\ee^{i\theta}\big]-\frac12\big(a+r\ee^{i\theta}\big)=\frac h2\big(1+r\ee^{i\theta}\big).$$

如果记$B$的坐标为$b$，因为$AB=AC$，所以$C$的坐标为$b\ee^{i\theta}$. 若记$z_2=\overrightarrow{BC}$，则

$$z_2=b\ee^{i\theta}-b=b\big(\ee^{i\theta}-1\big).$$

现在

$$
z_1\bar z_2=\frac h2\big(1+\ee^{i\theta}\big)b\big(\ee^{-i\theta}-1\big)\\
=\frac{bh}2\big(\ee^{-i\theta}-\ee^{i\theta}\big)=-i bh\sin\theta,
$$
...
**Proof:**
$z_1,z_2,z_3,z_4$四点共圆的充要条件是向量$z_1-z_3$和$z_1-z_4$的夹角等于向量$z_2-z_3$和$z_2-z_4$的夹角或互补（当$z_2$在$z_3$与$z_4$之间时），即
...
$z_1=r_1(\cos\theta_1+i\sin\theta_1),\cdots,z_n=r_n(\cos\theta_n+i\sin\theta_n)$
...
$$
(\cos\theta+i\sin\theta)^{-n}&=\frac1{(\cos\theta+i\sin\theta)^n}
=\frac1{\cos n\theta+i\sin n\theta}\\
...
$$z=\sqrt[\leftroot{-1}\uproot{2} n]{|w|}\bigg(\cos\frac{\theta+2k\pi}n+i\sin\frac{\theta+2k\pi}n\bigg)
,k=0,1,\cdots,n-1.$$

这$n$个复数恰好是以原点为中心、$\sqrt[\leftroot{-1}\uproot{2} n]{|w|}$为半径的圆的内接正$n$边形的顶点. 当$w=1$时， 若记$\omega=\cos\frac{2\pi}n+i\sin\frac{2\pi}n$，则$\sqrt[\leftroot{-1}\uproot{2} n]1$的$n$个值为
 ...
 **习题**\hypertarget{xiti1.2}{}
1. 把复数$z=1+\cos\theta+i\sin\theta$写成三角形形式.
...
4. 证明：$\triangle z_1z_2z_3$和$\triangle w_1w_2w_3$同向相似的充分必要条件为
...
5. 设$z_1\ne z_2$，证明：
  ...
  \begin{enuma}
   *  $z$位于以$z_1$和$z_2$为端点的开线段上，当且仅当存在$\lambda\in (0,1)$，使得
       $$z=\lambda z_1+(1-\lambda)z_2;$$
   *  $z$位于以$z_1$和$z_2$为端点的开圆弧上，当且仅当存在$\theta\,\big(0<|\theta|<\pi\big)$，使得
...
8.  $ABED,ACFG$是正方形， $AH\bot BC,M$是$DG$的中点.证明： $M,A,H$三点共线.
 
9. 在平行四边形$ABCD$中（见图 \ref{fig1.7}），如果
...
11. 设$z_1,\cdots,z_n$是单位圆周（以原点为中心、半径为$1$的圈周）上的$n$个点，如果$z_1,\cdots,z_n$是正$n$边形的$n$个顶点，证明：
...
15. \hypertarget{xiti1.2.15}{}设$z_1\ne z_2,0<\lambda\ne1$，证明由方程
...
16. 如果$z_1,\cdots,z_n$都位于过原点的直线的一侧，证明$\frac1{z_1},\cdots,\frac1{z_n}$也必位于该直线的某一侧，且满足

     $$z_1+\cdots+z_n\ne0,\quad\frac1{z_1}+\cdots+\frac1{z_n}\ne0.$$

17. 设$z_1,\cdots,z_n$是一个凸$n$边形的$n$个顶点，如果$a$满足关系
...
19. 设$0<\theta<\frac\pi2,P_m(x)=\sum_{k=0}^m(-1)^k\binom{2m+1}{2k+1}x^{m-k}$.证明：
...
20. 利用上题结果证明：
    ...
    *  $\prod_{k=1}^m\cot^2\frac{k\pi}{2m+1}=\frac1{2m+1}$.

 

# 扩充平面与复数的球面表示\label{sec1.3}}
...
$0\cdot \infty$和$\infty\pm\infty$都不规定其意义. 引进了$\infty$的复数系记为$\mathbb{C}_{\infty} $，即$\mathbb{C}_{\infty}=\mathbb{C}\cup\{\infty\}$.在复平面上，没有一个点和$\infty$相对应，但我们想像有一个\textbf{无穷远点}和$\infty$对应，加上无穷远点的复平面称为\textbf{扩充平面}或\textbf{闭平面}，不包括无穷远点的复平面也称为\textbf{开平面}.在复平面上，无穷远点和普通的点是不一样的，Riemann首先引进了复数的球面表示，在这种表示中，$\infty$和普通的复数没有什么区别. 设$S$是$\mathbb{R}^3$中的单位球面，即：
 ...
 **习题**
  ...
  *  证明：在复数的球面表示下，$z$和$\frac 1{\bar z}$的球面像关于复平面对称.

  *  证明：在复数的球面表示下，$z$和$w$的球面像是直径对点当且仅当$z\bar w=-1$.

  *  证明：在复数的球面表示下，$\mathbb{C}_\infty$中的点$z$和$w$的球面像间的距离为

     $$\frac{2|z-w|}{\sqrt{\big(|z|^2+1\big)\big(|w|^2+1\big)}}.$$

  *  证明：在复数的球面表示下，若$\begin{pmatrix}
  a&b\\c&d
  \end{pmatrix}$
  是二阶酉方阵，则$\mathbb{C}_\infty$的变换$w=\frac{az+b}{cz+d}$诱导了球面绕球心的一个旋转.

  *  证明：在复数的球面表示下，球面上的圆周对应于复平面上的圆周或直线，反之亦然.
  
  *  证明：在复数的球面表示下，复平面上两条光滑曲线在交点处的夹角与它们的球面像在交点处的夹角相等。
 

# 复数列的极限\label{sec1.4}}
复变函数是定义在平面点集上的复值函数，在讨论复变函数之前，必须对平面点集的知识有足够的了解.我们从复数列的极限谈起.

我们说$\mathbb{C}$中的复数列$\{z_n\}$收敛到$\mathbb{C}$中的点$z_0$，是指对于任给的$\varepsilon>0$，存在正整数$N$，当$n>N$时，$|z_n-z_0|<\varepsilon$，记作$\lim_{n\to\infty}z_n=z_0$.
...
设$z_n=x_n+i y_n,z_0=x_0+i y_0$，从等式
...
复数列$\{z_n\}$称为\textbf{Cauchy列}\index{C!Cauchy列}，如果对任给的$\varepsilon>0$，存在正整数$N$，当$m,n>N$时，有$|z_n<z_m|<\varepsilon$.设$z_n=x_n+i y_n,z_m=x_m+i y_m$，那么从等式
 ...
 **习题**
  ...
  *  设$z=x+i y\in\mathbb{C}$,证明：
     $$\lim_{n\to\infty}\bigg(1+\frac zn\bigg)^n=\ee^x(\cos y+i\sin y).$$
  *  证明：如果$\lim_{n\to\infty}z_n=z_0$，则
     $$\lim_{n\to\infty}\frac{z_1+z_2+\cdots+z_n}n=z_0.$$
  *  证明：若$\lim_{n\to\infty}z_n=z_0,\lim_{n\to\infty}w_n=w_0$，则
  ...
  *  设无穷三角阵
  ...
  满足
  \begin{enuma}
    *  对任意固定的$k$，$\lim_{n\to\infty}a_{nk}=a_k$存在；
    *  $\lim_{n\to\infty}\sum_{k=1}^na_{nk}$存在；
    *  $\sum_{k=1}^n|a_{nk}|\le M<\infty,\forall n\in \mathbb{N}$.
  \end{enuma}
  证明：若复数列$\{z_n\}$收敛，则$\lim_{n\to\infty}\sum_{k=1}^na_{nk}z_k$存在.
 

# 开集、闭集和紧集\label{sec1.5}}
...
由上面的定义可知，集$E$把复平面分成三个互不相交的部分：$\mathbb{C}=E^\circ\cup(E^{\textrm c})^\circ\cup\partial E$，即
$$\label{eq1.5.1}
...
这些集之间有下面的关系：
**Property: **\label{prop1.5.1}
  对于任意集$E$，有
  \begin{eenum}
    *  $a\in\bar E$的充要条件是对任意$\varepsilon>0$，有\label{prop1.5.1.1}
    $$\label{eq1.5.2}
       B(a,r)\cap E\ne\varnothing,
    $$
    这里，$\varnothing$表示空集；
    *  $(\bar E)^{\textrm c}=(E^{\textrm c})^\circ,\bar {E^{\textrm c}}=(E^\circ)^{\textrm c}$.\label{prop1.5.1.2}
  \end{eenum}
...
**Throrem: ** [（\textbf{Cantor}）]\index{D!定理!Cantor定理}\label{thm1.5.3}
  若非空闭集序列$\{F_n\}$满足中
  \begin{eenum}
    *  \label{thm1.5.3.1} $F_1\supset F_2\supset\cdots\supset F_n\supset\cdots$；
    *  \label{thm1.5.3.2} $\diam F_n\to 0$（当$n\to\infty$时），
  \end{eenum}
...
**Throrem: ** [（\textbf{Heine-Borel}）]\index{D!定理!Heine-Borel定理}\label{thm1.5.5}
...
$$
\begin{enum}
 *   \label{thm1.5.5.1}$F_n$是有界闭集；
 *   \label{thm1.5.5.2}$F_n\supset F_{n+1},n=1,2,\cdots$；
 *   \label{thm1.5.5.3}不能从$\mathscr F$中取出有限个开集来覆盖$F_n$；
 *   \label{thm1.5.5.4}当$n\to\infty$时, $\diam F_n\le\frac M{2^n}\sqrt2\to0$.
\end{enum}
$$
...
**Throrem: ** [（\textbf{Bolzano-Weierstrass}）]\index{D!定理!Bolzano-Weierstrass定理}
...
3.  指出下列点集的内部、边界、闭包和导集：
   \begin{enuma}
     *  $\mathbb{N}=\{k:k\text{为自然数}\}$；
     *  $E=\left\{\frac1k:k\text{为自然数}\right\}$；
     *  $D=B(1,1)\cup B(-1,1)$；
     *  $G=\{z\in\mathbb{C}:1<|z|\le2\}$；
     *  $\mathbb{C}$.
   \end{enuma}
4.  指出下列点集中哪些是开集，哪些是闭集，哪些是紧集：
   \begin{enuma}
     *  $\mathbb{Z}=\{k:k\text{为整数}\}$；
     *  $E$为有限集；
     *  $D=\{z\in\mathbb{C}:\Im z>0\}\backslash\big(\operatorname*{\cup}_{k=-\infty}^{\infty}F_k\big)$，其中，$F_k=\{z\in\mathbb{C}:z=k+\textrm  iy,0\le y\le1\}$；
     *  $G=B(0,1)\backslash\left\{\frac1{k+1}:k\text{为自然数}\right\}$；
     *  $\mathbb{C}\backslash B(\infty,R)$.
   \end{enuma}
5.  证明：若$D$为开集，则$D'=\bar D=\partial D\cup D$.
   *  设$\varLambda$是指标集，$\{D_\alpha\}_{\alpha\in\varLambda}$是开集族，$\{F_\alpha\}_{\alpha\in\varLambda}$是闭集族. 证明：$\operatorname*{\cup}_{\alpha\in\varLambda}D_\alpha$是开集，$\operatorname*{\cap} _{\alpha\in\varLambda}F_\alpha$是闭集.
6.  证明：有限个开集的交是开集，有限个闭集的并是闭集.
7.  设$D$是开集，$F\subset D$是非空紧集.证明：
   \begin{enuma}
     *  $d(F,\partial D)>0$；
     *  对任意$0<\delta<d(F,\partial D)$，存在$F$中的点$z_1,z_2,\cdots,z_n$，使得$F\subset\operatorname*{\cup}_{k=1}^nB(z_k,\delta)\subset D$，并且
       $$d\left(\operatorname*{\cup}_{k=1}^nB(z_k,\delta),\partial D\right)\ge d(F,\partial D)-\delta.$$
   \end{enuma}
8.  证明：若$E$是闭集，$F$是紧集，则存在$z_0\in E,w_0\in F$，使得$d(E,F)=|z_0-w_0|$.将$F$换成闭集后是否还存在这样的$z_0$和$w_0$?
9.  证明：若$E\subset \mathbb{C}$既是开集，又是闭集，则$E$是空集或$E=\mathbb{C}$.
 ...
 **习题**\hypertarget{xiti1.6}{}

1. 满足下列条件的点$z$所组成的点集是什么?如果是域，说明它是单连通域还是多连通域?
   \begin{enuma}
     *  $\Re z=1$；
     *  $\Im z<-5$；
     *  $|z-\textrm  i|+|z+\textrm  i|=5$；
     *  $|z-\textrm  i|\le|2+\textrm  i|$；
     *  $\arg(z-1)=\frac\pi6$；
     *  $|z|<1,\Im z>\frac12$；
     *  $\left|\frac{z-1}{z+1}\right|\le2$；
     *  $0<\arg\frac{z-\textrm  i}{z+\textrm  i}<\frac\pi4$.
   \end{enuma}
2. \hypertarget{xiti1.6.2}{}证明：非空点集$E\subset \mathbb{R}$为连通集，当且仅当$E$是一个区间.
3.  设$E$是非空点集，$A$是$E$的非空子集。若$A$是连通的，并且不存在$E$的连通子集真包含$A$，则称$A$是$E$的\textbf{连通分支}\index{J!集!连通分支}. 证明：开集的连通分支仍然是开集，闭集的连通分支仍然是闭集.
4.  设$E$是非空点集，$\varepsilon>0$.若对于$E$中的任意两点$a,b$，存在$E$中的有限个点$a=z_0,z_1,\cdots,z_n=b$，使得$|z_k-z_{k-1}|<\varepsilon$成立（$1\le k\le n$），则称$E$为$\varepsilon$- 连通的. 证明：紧集连通的充要条件是，对任意$\varepsilon>0$，它都是$ \varepsilon$- 连通的.并举例说明将紧集改为闭集后结论不再成立.
5.  证明：若$D$是有界单连通域，则$\partial D$连通.举例说明，若$D$是无界单连通域，则$\partial D$可能不连通.


# 复变函数的极限和连续性\label{sec1.7}}
...
设$z=x+i y$，用$u$和$v$记$w=f(z)$的实部和虚部，则有
$$w=f(z)=u(z)+i v(z)=u(x,y)+i v(x,y).$$
这就是说，一个复变函数等价于两个二元的实变函数$u=u(x,y)$和$v=v(x,y)$.

例如$w=z^2=(x+i y)^2=x^2-y^2+2i xy$，它等价于$u=x^2-y^2$和$v=2xy$两个二元函数；再如$w=|z|$，它等价于$u=\sqrt{x^2+y^2}$和$v=0$这两个二元函数.
...
设$a=\alpha+i\beta,z_0=x_0+i y_0,f(z)=u(x,y)+i v(x,y)$，由下面的不等式
$$
...
从上面的讨论知道，$f(z)=u(x,y)+i v(x,y)$在$z_0=x_0+i y_0$处连续的充要条件是$u(x,y)$和$v(x,y)$作为二元函数在$(x_0,y_0)$处连续.

紧集上的连续函数有许多重要的性质：
**Throrem: ** \label{thm1.7.1}
设$E$是$\mathbb{C}$中的紧集，$f:E\to\mathbb{C}$在$E$上连续，那么
\begin{eenum}
  *  \label{thm1.7.1.1} $f$在$E$上有界；
  *  \label{thm1.7.1.2} $|f|$在$E$上能取得最大值和最小值，即存在$a,b\in E$，使得对每个$z\in E$，都有
    $$|f(z)|\le |f(a)|,\quad |f(z)|\ge |f(b)|;$$
  *  \label{thm1.7.1.3} $f$在$E$上一致连续.
\end{eenum}
 ...
 **习题**\hypertarget{xiti1.7}{}

1. 证明：若$E$是紧集，$f:E\to\mathbb{C}$连续，则$f$在$E$上一致连续.
   *  证明：若$D$是单连通域，$0\notin D$，则必存在$D$上的连续函数$\varphi(z)$，使得$\varphi(z)\in \Arg z,\forall z\in D$. $\varphi(z)$称为$\Arg z$在$D$上的一个\textbf{单值连续分支}\index{D!单值连续分支}.
2.  证明：若$E$是紧集，$f$在$E$上连续，则$f(E)$也是紧集.将紧集换成闭集，结论是否成立?
3.  设$f$是域$D$上的连续函数，并且对任意$z_0\in\partial D,\lim_{z\to z_0}f(z)$存在，证明：
   $$F(z)=\begin{cases}
   f(z)&z\in D;\\
   \lim_{\zeta\to z}f(\zeta),&z\in \partial D
   \end{cases}$$
   在$\bar D$上连续.
4.  证明：若$f$在域$D$上一致连续，则对任意$z_0\in\partial D,\lim_{z\to z_0}f(z)$存在.
5.  研究$f(z)=\frac1{1-z}$和$g(z)=\frac1{1+z^2}$在$B(0,1)$上的连续性与一致连续性.
6.  \hypertarget{xiti1.7.7}{} 设连续映射$f:E\to\mathbb{C}$满足
...
7.  证明：
   \begin{enuma}
     *  存在连续映射将$[0,1]$映为$\partial B(0,1)$；
     *  不存在一一连续映射将$[0,1]$映为$\partial B(0,1)$.
   \end{enuma}
8.  设$f$是域$D$上的复变函数，$z_0\in\partial D$.若$\lim_{z\to z_0}f(z)=A$存在，
